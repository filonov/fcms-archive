...
class DMZ_Nestedsets
{
    ...
    /**
     * For using as url path. Same as $_nodename
     * 
     * @var string
     * @access private
     */
    ...
    public function dump_tree($object, $attributes = NULL, $type = 'array', $skip_root = TRUE)
    {
        if ($this->is_valid_node($object))
        {
            ...
            foreach ($result as $key => $value)
            {
                ...
                // DVF: create the relative alias path to this node
                $result[$key]['alias_path'] = '';
                if (!empty($this->_alias))
                {
                    $alias_path[$level] = $value[$this->_alias];
                    for ($i = 0; $i <= $level; $i++)
                    {
                        $result[$key]['alias_path'] .= $alias_path[$i] . '/';
                    }
                }

                // DVF: create the relative path from id to this node
                $result[$key]['__id_path'] = '';
                if (!empty($this->_nodename))
                {
                    $idpath[$level] = $value['id'];
                    for ($i = 0; $i <= $level; $i++)
                    {
                        $result[$key]['__id_path'][$i] = $idpath[$i];
                    }
                }
            }
